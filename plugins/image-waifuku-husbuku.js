...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if (!text) throw `Masukkan nama waifu mu contoh nya::\n${usedPrefix}${command} Chizuru`
	...
	//await conn.sendButton(m.chat, `Ini Waifu mu ${text.capitalize()}`, await shortUrl(res), res, [['Next', `${usedPrefix + command} ${text}`]], m)
	await conn.sendButton(m.chat, `Ini Waifu mu ${text.capitalize()}\n`, 'waifu mu lumayan juga mas, tapi cantikan Chizuru 🥰', res, [['Next', `${usedPrefix + command} ${text}`]], m)
}
handler.help = handler.alias = ['waifuku <nama waifumu>', 'husbuku <nama waifumu>']
handler.tags = ['anime']
handler.command = /^(waifuku|husbuku)$/i
